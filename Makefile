...
build: ${INCLUA} ${CMAKE_BUILDFLAGS} ${CMAKE_BIN} ${CMAKE} ${PROJECT_VERFILE}
	${CMAKE} --build ${CMAKE_BIN} ${CMAKE_BUILDFLAGS}

test: build ${LIBLUA} ${CTEST} ${CTEST_FLAGS}
	cd ${CMAKE_BIN} && ${CTEST} ${CTEST_FLAGS}

install: ${CMAKE_BIN} build ${CMAKE} ${INSTALL_PREFIX}
...
${CMAKE_BIN}: ${CMAKE_GENFLAGS} ${INCLUA} ${LIBLUA} \
	...
	${CMAKE_C_COMPILER} ${CMAKE_CXX_COMPILER} ${PROJECT_VERFILE}
...
${PROJECT_VERFILE}: ${PROJECT_MAJ} ${PROJECT_MIN}
	echo "#ifndef LuaCXX_VERSION_FILE\n \
	#define LuaCXX_VERSION_FILE\n \
	#define LUACXX_MAJOR ${PROJECT_MAJ}\n \
	#define LUACXX_MINOR ${PROJECT_MIN}\n \
	#endif\n \
	\n" > ${PROJECT_VERFILE}
${CHANGELOG}: ${BINLUA} ${PROJECT_MAJ} ${PROJECT_MIN}
	${BINLUA} changelog_gen.lua ${PROJECT_MAJ}.${PROJECT_MIN}